...
<?php } ?>
  ...
  <div>
    ...
    <p class="text-body-secondary d-flex justify-content-center py-1">Gerência avalia Competência Geral</p>
  </div>
  ...
  <?php  
          ...
          $pergunta_id = $perguntas_fetch['colab'];
  ...
  <div class="container py-2">
    <form action="/funcoes/envio_lid_gerencia.php" method="post">
    <div>
    <input name="id_tipo" value="7" type="hidden" id="id_tipo" checked required>
      ...
      <div class="mb-3">
        ...
        <option value='<?php echo "idusuario:" . ($idusuario1 ?? '') . ", setorid:" . ($setorid1 ?? '') ?>'><?=($nomecompleto1 ?? 'Selecione')?></option>
          ...
          $delega_tipo = $delega_acoes['geren_reter'];
          }
          ...
          and e.idusuario not in($delega_tipo)
          and e.idusuario $id_usu_sheila  
